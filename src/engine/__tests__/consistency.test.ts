...
describe('不変条件: 場所移動は常にlocationIdとvisitedByを同時更新する', () => {
  ...
  it('initializeWorldState: NPC初期配置でlocationIdのみ設定、visitedByには追加しない', () => {
    const scenario = createSampleScenario()
    ...
    expect(state.locationStates['loc-entrance'].visitedBy).not.toContain('npc-misaki')
    // npc-sato is at loc-basement
    expect(state.actorStates['npc-sato'].locationId).toBe('loc-basement')
    expect(state.locationStates['loc-basement'].visitedBy).not.toContain('npc-sato')
  })
